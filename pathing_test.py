...
class EuclideanEdgeWeightedGraph:
    ...
    def add_edge_xy(self, x,y,x2,y2):
        ...
        self.adj[self.vertices.index(to)].append(self.vertices.index(fromnode))
        self.parents[fromnode] = to 
        self.weights[self.vertices.index(fromnode)] = math.hypot((x2-x), (y2-y))
        self.path_costs[(x2,y2)] = self.weights[self.vertices.index(fromnode)] + self.path_costs[(x,y)]
    ...
    def update_cost_xy(self, x, y): 
        queue = deque([])
        v = self.vertices.index((x,y))
        cost = self.weights[v]
        elem = (x,y)
        queue.appendleft(self.parents[(x,y)])
        while queue and self.parents[elem] is not None:
            elem = queue.pop()
            vertex = self.get_vertex(elem[0], elem[1])
            cost += self.weights[vertex]
...
class Obstacle:
    ...
    def __init__(self, width, length, center_x, center_y, shape):
        self.width = width
        self.length = length
    ...
    def is_in_equation(self, x, y):
        if self.type == "square":
            if math.fabs(x - self.center_x) <= self.width and math.fabs(y - self.center_y) <= self.length:
                return True
            else:
                return False
        elif self.type == "circle":
            if (x-self.center_x)**2 + (y-self.center_y)**2 <= self.width:
                return True
            else:
                return False


obstacle = Obstacle(1, 2, 1, 2, "circle")


class RRTStar:

    def __init__(self, w, x, y, z, n, r, obstalce_list):
        self.w = w
        self.x = x
        self.y = y
        self.z = z
    ...
    def planning(self):
        ...
        for i in range(self.n):
            ...
            if not self.collision_detection( x_new, y_new) and (x_new, y_new) != (self.x, self.z):
                ...
                if self.collision_detection_eq(node[0], node[1], x_new, y_new):
                    if self.calc_cost(self,x_new, y_new, self.x, self.z) < self.r: 
                        self.rrt_graph.add_edge_xy(node[0], node[1], x_new, y_new)
                        self.rrt_graph.add_edge_xy(x_new, y_new, self.x, self.z)                    
                        break 
                    else:  
                        self.steer(node[0], node[1], x_new, y_new)
                        #self.rrt_graph.add_edge_xy(node[0], node[1], x_new,y_new)
                    for i in range(len(self.rrt_graph.vertices)):
                        if cost_list[i] < self.r and cost_list[i]!= node:
                            nearest_vertex.append(self.rrt_graph.vertices[i])
                    self.rewire(x_new, y_new, nearest_vertex)
    ...
    def nearest(self, x, y):
        ...
        for key, value in point_costs.items():
            if value == min_cost: 
                graph_node = key
                break 
        if graph_node == (self.x, self.z): 
            list1 = list(point_costs.values())
            i = list1.index((self.x, self.z))
            key = list(point_costs.keys())[i]
            del point_costs[key]
            min_cost = min(list(point_costs.values()))
            for key, value in point_costs.items():
                if value == min_cost: 
...
def main():
    ...
    rrt = RRTStar(0, 20, 0, 10, 3000, 1, obstacle_list)
    result = rrt.planning()
    xarray = []
    yarray = []
    x1arr = []
    ...
    circle2 = plt.Circle((8,10),2,color = 'r')
